...
class AssetsViewModel: ObservableObject {
	...
	//Here are some wallets but feel free to try with yours.
	//Small (4 NFTs) = 0x1b584fc86390d7D83B529a4346330ee3D2061681
	//Medium (27 NFTs) = 0x3af395f085Ac04Dc4Fc26C247D6B8Bf11b282B8A
	//Big (129 NFTs) = 0x866becf9CfcAdABf3642146af35A3bNukeE92A3833ee
	//Gigantic (630 NFTs) = 0x1b6db55c7aa95f74f38b27d50eaee75a6a27d631
	@Published var wallet = "0x1b584fc86390d7D83B529a4346330ee3D2061681"
	
	var sortedAssets: [Asset] {
		if (headerValues.isFloorValues) {
			return assets.sorted{ $0.floorPrice > $1.floorPrice }
		} else {
			return assets.sorted{ $0.averagePrice > $1.averagePrice }
		}
	}
	...
	//MARK: - START and END of synchronization here
	func startSync() {
	...
	func getData(completion: @escaping (Result<[Asset], OpenSeaError>) -> ()) {
		...
		self.getAssets([], nil, wallet) {  [weak self] result in
			...
			case .success(let assets):
				...
				DispatchQueue.main.async {
					self.headerValues.progress = 0.15
				}
		...
		openSeaService.getAssets(wallet, cursor) { [weak self] result in
			...
			case .success(let value):
				...
				value.assets.forEach {
					...
					if let id = $0.id,
					   ...
					   let tokenId = $0.tokenId {
						...
						if let index = currentAssets.firstIndex(where: { $0.collectionSlug == collectionSlug }) {
							
							let insertionIndex = currentAssets[index].nfts.insertionIndexOf(newNFT) { $0.id < $1.id }
							currentAssets[index].nfts.insert(newNFT, at: insertionIndex)
						} else {
				...
				if (maxReached) {
					...
					self.getAssets(currentAssets, nextCursor, wallet) { result  in
						switch result {
	...
	func getValues(_ assets: [Asset], completion: @escaping ([Asset]) -> ()) {
		...
		DispatchQueue.global(qos: .userInitiated).async {
			...
			for (i, asset) in updatedAssets.enumerated() {
				...
				DispatchQueue.main.async {
					self.headerValues.progress = 0.15 + (0.85 / Double(updatedAssets.count) * Double(i))
				}
